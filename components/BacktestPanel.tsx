...
const BacktestPanel: React.FC = () => {
  ...
  const runBacktest = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/backtest?start_date=${startDate}&end_date=${endDate}`);
      const data = await response.json();
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
              ...
              <div className={`text-2xl font-bold ${results.metrics.pnl_total >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {results.metrics.pnl_total >= 0 ? '+' : ''}{results.metrics.pnl_total.toFixed(2)}
              </div>
              <div className="text-xs text-slate-400 mt-1">
                {results.metrics.avg_pnl_per_trade.toFixed(2)} bps/trade
              </div>
            </div>
            ...
            <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
              ...
              <div className="text-xs text-slate-400 mt-1">
                Max DD: {results.metrics.max_drawdown.toFixed(2)} bps
              </div>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={results.equity_curve}>
                ...
                <YAxis 
                  stroke="#64748b"
                  tick={{ fill: '#94a3b8', fontSize: 11 }}
                  label={{ value: 'P&L (bps)', angle: -90, position: 'insideLeft', fill: '#94a3b8' }}
                />
                <Tooltip 
                  contentStyle={{ 
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-900 text-xs uppercase text-slate-400 sticky top-0">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">P&L (bps)</th>
                  </tr>
                </thead>
